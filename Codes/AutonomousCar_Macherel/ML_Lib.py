...
def showImage(title, frame, show=False):
    if show:
        #cv2.cvtColor(self.roadFollower.drawed_img, cv2.COLOR_RGB2BGR)
        cv2.imshow(title,cv2.cvtColor(frame, cv2.COLOR_RGB2BGR))
        # plt.imshow(frame)
        # plt.title(title)


def drawHeadingLine(img, steering, line_color=(0, 0, 255), line_width=5, ):
